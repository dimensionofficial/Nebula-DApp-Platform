...
// 国家边框颜色
const HOVER_COUNTRY_COLOR = 0x00ffff;
...
const Shaders = {
  ...
  atmosphere: {
    ...
    fragmentShader: [
      ...
      'gl_FragColor = vec4( 0, 0.666, 1.0, 1.0 ) * intensity;', // 地球边框蒙层颜色(r, g, b, a)
      '}',
    ].join('\n'),
  },
};
...
class GlobeRenderer extends EventEmitter2 {
  ...
  init() {
    ...
    // 背景图片
    {
      ...
      material.map = THREE.ImageUtils.loadTexture('../meetup/sky.jpg');
    ...
    // 地球
    {
      ...
      uniforms.texture.value = THREE.ImageUtils.loadTexture('../meetup/earth.png');
    ...
    // Cloud
    {
      ...
      const material = new THREE.MeshBasicMaterial({
        color: 0x00ffff,
        alphaMap: THREE.ImageUtils.loadTexture('../meetup/Clouds.jpg'),
  ...
  methods: {
    renderPrice(priceMap2) {
      ...
      this.pointSeries = countryPointsJson
        ...
        gogood(){
      this.$router.push({ path: '/goodDetail',query:{gid:this.detail.gid}})
    },
    goodDetail(index){
      this.$router.push({ path: '/goodDetail',query:{gid:this.good[index].gid}})
    },
    kanjia(){
      var that =this;
      that.dialogVisibleb=true;
    },
    detailless(){
      var that = this;
      that.detailnum = 3;
    ...
    myBargain(){
      this.$router.push({ path: '/myBargain',query:{uid:this.$route.query.uid}})
    },
    detailall(){
      var that =this
      that.detailnum = 999;
    ...
    bangkan(){
      ...
      price:that.detail.priceList[that.detail.members.length],
      });
      that.axios({
      method: 'POST',
      url: 'http://192.168.1.212:8086/order/reduce',
      headers:{"Content-Type":"application/x-www-form-urlencoded"},
      data: postData}).then((res)=>{
        if(res.data.code == 200){
           that.axios.get('http://192.168.1.212:8086/order', {
        　　params: {oid:that.$route.query.oid}
            }).then(function (response) {
              that.detail = response.data.data
              console.log(that.detail)
              that.$store.dispatch('getdetail',response.data.data);
              that.kuan = (that.detail.price-that.detail.curPrice)/that.detail.price*100;
              })   
          this.$router.push({ path: '/bargainDetail',query:{oid:res.data.data.oid,uid:that.$router.query.uid}})
        }
      });
      that.meCut = true
      that.dialogVisiblea = true
      },
    launch(index){
      this.$router.push({ path: '/launch',query:{gid:this.good[index].gid,uid:this.$route.query.uid}})
    },
    golaunch(){
      this.$router.push({ path: '/launch',query:{gid:this.detail.gid,uid:this.$route.query.uid}})
    },
