...
export default new Vuex.Store({
  ...
  actions: {
    ...
    async connectScatterAsync({ commit, dispatch, state }) {
      console.log('from store/index.js line 100 Connecting to Scatter desktop...');
      // const connected = true;
      const connected = await getApi(Global.contractType).api.connectScatterAsync();
      console.log('from store/index.js line 103 Connect Scatter result: ', connected);
      if (connected) {
        commit('setIsScatterConnected', true);
        if (getApi(Global.contractType).currentEOSAccount()) {
          console.log('from store/index.js line 107 currentEOSAccount: ', getApi(Global.contractType).currentEOSAccount());
    ...
    async updateLandInfoAsync({ commit, state }) {
      ...
      try {
        ...
        console.log('updateLandInfoAsync------from store/index.js line 140', new Date().getTime());
        rows.forEach((row) => {
    ...
    async updateMarketInfoAsync({ commit, state }) {
      try {
        ...
        marketInfo.supply = `${(parseFloat(marketInfo.supply.split(' ')[0]) - 40000000).toDecimal(4).toString()} EON`;
...
// -----------------------------------------------------------------------
    async getMyStakedInfo({ commit, state }) {
      try {
        ...
        stakedInfoList[0].refund = (refund.amount || '0 EON');
    ...
    async loginScatterAsync({ commit, dispatch, state }) {
      ...
      try {
        ...
        console.log('from store/index.js line 220: ',identity);
        if (!identity) {
